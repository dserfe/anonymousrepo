...
class CFGBuilder(ast.NodeVisitor):
    ...
    def build(
        self, name: str, tree: Module, allfunctioncfgs: dict, asynchr: bool = False, entry_id: int = 0, 
    ) -> CFG:
        """
        ...
        """
        self.cfg = CFG(name, allfunctioncfgs, asynchr=asynchr, short=self.isShort)
    ...
    def new_classCFG(self, node: ClassDef, asynchr: bool = False) -> None:
        ...
        self.cfg.classcfgs[node.name] = classcfg = class_builder.build(
            node.name, class_body, self.allfunctioncfgs, asynchr, self.current_id
        )
        self.allfunctioncfgs[node.name] = classcfg
    ...
    def new_functionCFG(self, node: FunctionDef, asynchr: bool = False) -> None:
        ...
        cfg = self.cfg.functioncfgs[node.name] = func_builder.build(
            node.name, func_body, self.allfunctioncfgs, asynchr, self.current_id,
        )
        self.allfunctioncfgs[node.name] = cfg
        # print(self.allfunctioncfgs)
        # exit(0)
